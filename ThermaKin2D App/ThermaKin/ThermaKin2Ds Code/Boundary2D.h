...
class Bound2D
   /* This class calculates mass and heat flows through a surface of a 2D object. Mass flux 
	  ...
	  convective or radiative, is turned on, provided that the flame start time has come and 
	  the flame length is positive. This heat flux (or outT, in the case of convection) depends 
	  on position with respect to flame bottom and total normalized burning rate of the 
	  boundary. Flame bottom position, fpos, is defined as that of ignited surface element 
	  closest to the object bottom. Total normalized burning rate [m] is defined as the sum of 
	  products of ignition criterions and lengths of all surface elements of the boundary. 
	  Inside flaming region, flame heat flux is defined by two constants, val0 and val1, which 
	  are heat fluxes (or outT) below and above specified distance from flame bottom. Below 
	  flaming region, the heat flux is expressed as Gaussian function of distance from flame 
	  bottom with the maximum value of val0. Length of flame, fhgt, is set to be equal to a 
	  constant plus factored power law function of total normalized burning rate. Above flaming 
	  region, at distance pos from the object bottom, the heat flux is equal to 
	  (val0 or val1)*fct*exp(-ln(fct)*((pos-fpos+ad)/(fhgt+ad))^2), where fct and ad are 
	  constants. Constant background temperature value is added to outT describing external and 
	  flame convective heat fluxes. This value is also used to compute extra (background) 
	  radiative heat flux onto the object surface. massflow function uses results of volume 
	  calculation stored in the properts buffer of surface element; it should be applied to all 
	  surface elements of the boundary, from bottom to top, prior to heat flow calculations. 
	  inp_indpElem function returns false after all radiation is absorbed. When properts buffer 
	  is defined for the radiation absorbing element, heatflow function uses results of volume 
	  calculation stored in that buffer. The mass and heat flows (including re-radiation) are 
	  added up for all surface elements and reported by report function. A capability to ramp 
	  up or down the external heat fluxes by scaling them with a factor between 0 and 1, which 
	  is a linear function of time, has been added to this class. */
{
public:
