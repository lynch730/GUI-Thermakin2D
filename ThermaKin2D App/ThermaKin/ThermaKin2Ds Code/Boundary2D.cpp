...
bool Bound2D::load(std::istringstream& boundpar,std::string name)
{
	...
	if(buf=="YES")
	{
		if (!((boundpar>>buf)&&(buf=="START")&&(boundpar>>buf)&&(buf=="TIME:")&&
			  (boundpar>>fL_tme)&&(boundpar>>buf)&&(buf=="IGNITION")&&(boundpar>>buf)&&
			  (buf=="MASS")&&(boundpar>>buf)&&(buf=="FLUXES:")&&(boundpar>>buf)))
		{ return false; }
		...
		if(!((boundpar>>buf)&&(buf=="HEAT")&&(boundpar>>buf)&&(buf=="FLUX")&&
			 ...
			 (boundpar>>fhFabv_fct[0])&&(boundpar>>fhFabv_ad)&&(boundpar>>buf)&&
...
double Bound2D::heatflow(double tme,bool calc_der)
{
	...
	if((tme>=fL_tme)&&(ign_pos>=0.0))
	{
		double sfpos=surf_pos-ign_pos;
		double flhgt=fL_val+fL_fct*pow(totl_IGN,fL_pow);
		
		if(flhgt>0.0)
		{
			if(sfpos<0.0) { heat=fhFins_val[0]*exp(-fhFbel*pow(sfpos,2)); }
			else if(sfpos<flhgt) { heat=((sfpos<fhFins_pos) ? fhFins_val[0] : fhFins_val[1]); }
			else
			{
				if(sfpos<fhFins_pos) { heat=fhFabv_fct[0]*exp(-fhFabv_pow*
					                             pow((sfpos+fhFabv_ad)/(flhgt+fhFabv_ad),2)); }
				else { heat=fhFabv_fct[1]*exp(-fhFabv_pow*
					                             pow((sfpos+fhFabv_ad)/(flhgt+fhFabv_ad),2)); }
			}
...
void Bound2D::report(std::ostream& output,bool header)
{
	...
	if(header)
	{
		output<<"BOUNDARY\tAREA [m^2]   \tFLAME POS [m]\tHEAT FLOW IN [W]\tMASS FLOW OUT [kg/s]:";
