#include "2Dax_Model.h"

void TwoDax_model::zero()
{
	novoid=true;
	LES.init_setup(0,0);
	NmT3=NmT3min1=0;
	...
	{
		Num_Elem[c]=0;
		Colm_pos[c]=Colm_hgt[c]=0.0;
		frntB_hF[c]=backB_hF[c]=0.0;
	}
...
void TwoDax_model::load(std::istringstream& conds)
{
	zero();
	...
	while(read)
	{
		if((buf=="OBJECT")&&(conds>>buf))
		{
			if((buf=="STRUCTURE")&&(conds>>buf))
			{
				...
				{
					if((buf=="FROM")&&(conds>>buf)&&(buf=="AXIS:")&&(conds>>buf))
					{
						cl=Consts::MAX_COLM;
						while(read&&(buf!="OBJECT")&&(buf!="INTEGRATION"))
						{
							if((buf=="LAYER")&&(conds>>buf)&&(buf=="WIDTH:")&&(conds>>LW))
							{
		...
		if(Num_Elem[b])
		{
			while(object2D[b].begin()->Ydim>0.0)
			{
				if(Num_Colm<cl)
				{
					...
					for(E=object2D[b].begin(); E!=object2D[b].end(); E++)
					{
	...
	for(cl=0; cl<Num_Colm; cl++)
	{
		...
		{
			E->Zdim=Colm_pos[cl];
			for(int c=0; c<mat.Ncomps(); c++) { E->massT[c]*=Colm_pos[cl]; }
		}
	}
	
	double size_tol=Elem_setSize/double(Consts::MAX_ELEM_COLM*5);
...
void TwoDax_model::report(std::ostream& output)
{
	output<<"**************************************************************************"
		  <<"\n\nTime [s] =\t"<<tme<<"\n\n";
			...
			if((clo==outfreq_Colm)||(cl==(Num_Colm-1)))
			{
				...
				while(E!=object2D[cl].end())
				{
					...
					if(Eo==outfreq_Elem)
					{
						...
						for(c=0; c<mat.Ncomps(); c++) { output<<"\t"<<(E->massT[c]/(E->Xdim*E->Ydim*E->Zdim)); }
					}
				...
				output<<"\n-----------------------------\nThickness [m] =\t"<<Colm_hgt[cl]
					  <<"\nMass [kg/m^2] =\t"<<(mass/(object2D[cl].begin()->Ydim*object2D[cl].begin()->Zdim))
					  <<"\n";
			}
	...
	output<<"\nTotal radius [m]  =\t"<<Totl_Size
		  <<"\nTotal mass [kg] =\t"<<(tot_mass*6.28318530718)<<"\n\n";
}

void TwoDax_model::grid_and_rates()
{
	extern Comps mat;
	...
	for(int cl=0; cl<Num_Colm; cl++)
	{
		...
		{
			--E;
			E->Xdim=mat.volume(*E)/(E->Ydim*E->Zdim);
			while(E!=object2D[cl].begin())
			{
				if(E->Xdim<Elem_minSize)
				{
					...
					tmpEto.Xdim=mat.volume(tmpEto)/(E->Ydim*E->Zdim);
			...
			object2D[cl].front().Xdim=mat.volume(object2D[cl].front(),true)/
				                      (object2D[cl].front().Ydim*object2D[cl].front().Zdim);
			if(!object2D[cl].front().properts) { object2D[cl].front().setup_prop_buf(); }
			mat.export_buf(object2D[cl].front());
			Colm_hgt[cl]=object2D[cl].front().Xdim;
			
			if(Num_Elem[cl]>1)
			{
				object2D[cl].back().Xdim=mat.volume(object2D[cl].back(),true)/
					                     (object2D[cl].back().Ydim*object2D[cl].back().Zdim);
				if(!object2D[cl].back().properts) { object2D[cl].back().setup_prop_buf(); }
				mat.export_buf(object2D[cl].back());
				Colm_hgt[cl]+=object2D[cl].back().Xdim;
				
				++E; En=object2D[cl].end(); --En;
				while(E!=En) { E->Xdim=mat.volume(*E)/(E->Ydim*E->Zdim); Colm_hgt[cl]+=E->Xdim; ++E; }
			}
			
			if(cl>0)
			{
				slope=sqrt(pow((Colm_hgt[cl-1]-Colm_hgt[cl]),2)+pow((Colm_pos[cl]-Colm_pos[cl-1]),2))/
					  (Colm_pos[cl]-Colm_pos[cl-1]);
			}
			else { slope=1.0; }
			
			frntB.massflow(&object2D[cl].front(),'X',slope,true);
			backB.massflow(&object2D[cl].back(),'X',1.0,true);
		}
		else { Colm_hgt[cl]=0.0; }
	}
	...
	for(int cl=0; cl<Num_Colm; cl++)
	{
		...
		if(Num_Elem[cl]>1)
		{
			frntB.inp_surfElem(&object2D[cl].front(),'X',slope,Colm_pos[cl],Colm_hgt[cl]);
			E=object2D[cl].begin();
			do { ++E; } while((E!=object2D[cl].end())&&frntB.inp_indpElem(&(*E)));
			frntB_hF[cl]=frntB.heatflow(tme,true);
			
			backB.inp_surfElem(&object2D[cl].back(),'X',1.0,Colm_pos[cl],0.0);
			E=object2D[cl].end(); --E;
			...
			if(frntB.massFlux_Num()||backB.massFlux_Num())
			{
				...
				while(En!=object2D[cl].begin())
				{
					...
					En->Xdim=mat.volume(*En,true)/(En->Ydim*En->Zdim);
		...
		else if(Num_Elem[cl]>0)
		{
			...
			backB.inp_surfElem(&object2D[cl].front(),'X',1.0,Colm_pos[cl],0.0);
			backB_hF[cl]=backB.heatflow(tme,true);
