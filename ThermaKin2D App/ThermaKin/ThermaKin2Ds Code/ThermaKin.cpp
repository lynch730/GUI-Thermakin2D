...
int main()
{
	...
	if(IO.read_files())
	{
		...
		else if(model=="2Dax")
		{
			...
			catch (TwoDax_model::loadErr err)
			{
				IO.out << "Object type:  2Dax"
					<< "\nNumber of layers:  " << TwoDax.Num_layers()
					<< "\n\nFront Boundary"
					<< "\nExternal heating:  " << (TwoDax.frntBound_extHeat() ? "on" : "off")
					<< "\nMass transport:  " << (TwoDax.frntBound_massflow() ? "on" : "off")
					<< "\n\nBack Boundary"
					<< "\nExternal heating:  " << (TwoDax.backBound_extHeat() ? "on" : "off")
					<< "\nMass transport:  " << (TwoDax.backBound_massflow() ? "on" : "off")
					<< "\n\n";
